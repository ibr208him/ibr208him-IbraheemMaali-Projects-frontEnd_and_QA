...
export default function Input({type,name,id,title,value,onChange,errors,onBlur,touched,ref}) {
...
id!="password"?(
  ...
  { title?
  <label className=' fw-bold text-white mb-2' htmlFor={id}>{title}</label>
  :
  ''
   }
  <input type={type} name={name} id={id} className='form-control w-100' value={value} onChange={onChange} onBlur={onBlur} ref={ref}/>
...
''
 ...
 <div className='passwordDiv d-flex align-items-center w-100'>
<input type={visible?'text':'password'} name={name} id={id} className='password-input form-control w-100 border-none  focus:border-none focus:outline-none' value={value} onChange={onChange} onBlur={onBlur} ref={ref}/>
<div className='eye-icon border-none' onClick={()=>setVisible(!visible)}>
  {!visible?
